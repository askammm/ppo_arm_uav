...
gnome-terminal --tab --title="n3" -- bash -c "docker exec -it -w \"$ROOT_PATH\" \"$CONTAINER_NAME\" bash -lc 'source devel/setup.bash && echo ROS DISTRO: \$ROS_DISTRO && roslaunch residual_ppo fixed_schedule_experiment.launch  ppo_enabled:=true  run_label:=ppo_on'; exec bash "
